...
public class Main {
	// FIXES
	// check for exceptions - character input,wrong acct number
	// joint accts
	// display primary and secondary users
	// wrong amt of money
	// make it to where you cant withdraw or deposit from acct number not listed
	...
	private static void showCustomerMenu(String userName) {
		...
		do {
			...
			case 2: {
				...
				// if (!activity.validateAccount(currentUserId, an)) {
			...
			case 3: {
				...
				// }
				// } else {
				// System.out.println("Secondary user does not exist. Please Register as a
				// User.");
				// }
		...
		do {
			...
			case 7:
				...
				if (noOfAccounts > 0) {
					...
					sc.nextLine();
					if (an > 0) {
						activity.changeAccountActiveStatus(an, false);
