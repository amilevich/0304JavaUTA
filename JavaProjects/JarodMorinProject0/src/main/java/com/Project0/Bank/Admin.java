...
public class Admin extends Employee {
	...
	@Override
	void openUserPortal(Bank banky, Scanner scanner) { // this method displays a menu to adminss that log in that
			...
			if (banky.accountMap.containsKey(accnum)) {
				...
				while (true) {
					...
					if (amount > banky.accountMap.get(accnum).getBalance()) {
						...
						banky.accountMap.get(accnum).setBalance(banky.accountMap.get(accnum).getBalance() - amount);
						banky.accountMap.get(accnum2).setBalance(banky.accountMap.get(accnum2).getBalance() + amount);
