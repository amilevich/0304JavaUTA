...
public class AppDriver {
	...
	static void MainMenu() throws WithdrawTooLargeException, NoAccountDetectedException, TooManyAccountsException, NotApprovedException 
	{
		...
		default:{System.out.println("Please choose one of the given options"); System.out.println("\n"); MainMenu(); break;}}
	...
	{ //	logged in customer can perform their function after giving respective inputs
		...
		case 1: {
			...
			}}catch(TooManyAccountsException e){e.printStackTrace();System.out.println("\n");CustMenu(cus);}
			System.out.println("\n"); CustMenu(cus);
			break;}
		...
		case 3:{
			...
			throw new NoAccountDetectedException("You must make an account to proceed");
				}}catch(NoAccountDetectedException e){e.printStackTrace();System.out.println("\n");CustMenu(cus);}		
			break;}
		...
		case 4:{
			...
			if (choices >= 1 && choices <=5) {
				if (acc.get(choices - 1).getAccount_State()<1) {throw new NotApprovedException("This account isn't approved or has been denied");}
						...
						}else {System.out.println("Not an option");System.out.println("\n");CustMenu(cus);}
					}else {System.out.println("This User doesn't exist, Going back to menu");System.out.println("\n"); CustMenu(cus);}
						}}
			else {
			...
			CustMenu(cus);	
					...
					catch(NotApprovedException e) {e.printStackTrace();System.out.println("\n"); CustMenu(cus);}
			break;			
		}
		...
		case 5:{
			...
			System.out.println("You need to make a valid choice");System.out.println("\n");
			CustMenu(cus);	
			}
			}	
			else {throw new NoAccountDetectedException("You need to make an account to deposit");
		...
		case 6:{
			...
			CustMenu(cus);	
					...
					catch(WithdrawTooLargeException e){e.printStackTrace();System.out.println("\n");CustMenu(cus);}
					catch(NotApprovedException e) {e.printStackTrace(); System.out.println("\n"); CustMenu(cus);}
			break;}
		
		case 7:{
				System.out.println("Thanks for your business, Have a good one"); System.out.println("\n"); MainMenu(); break;
		}
		
		default:{System.out.println("Not an Option, going back to menu");System.out.println("\n"); CustMenu(cus); break;}
			}
	...
	static void EmpMenu(Employee emp) throws WithdrawTooLargeException, NoAccountDetectedException, TooManyAccountsException, NotApprovedException 
	{	//logged in employee can execute its functions
		...
		case 1: {
			...
			for (Accounts account : masterlist) {
				System.out.println(" " + ++i + "\tAccNumber: " + account.getAutoGeneratedNum() + "\tAccUser: "
						...
						+ account.getBalance() + "\tAccState: " + account.getAccount_State()); System.out.println();
			}
			System.out.println("\n");
			EmpMenu(emp);
			break;
		...
		case 2: {
			...
			for (Accounts A : accounts) { System.out.println(" " + ++i + "\tAccNumber: " + A.getAutoGeneratedNum() + "\tAccUser: "
					...
					+ A.getBalance() + "\tAccState: " + A.getAccount_State());System.out.println();}
			...
			switch(d) {
			case 1:{accounts.get(c-1).setAccount_State(1);ADI.updateAccount(accounts.get(c-1)); System.out.println("Account updated\n"); Log.info("Account updated");EmpMenu(emp);break;}
			case 2:{accounts.get(c-1).setAccount_State(-1);ADI.updateAccount(accounts.get(c-1)); System.out.println("Account updated\n"); Log.info("Account updated");EmpMenu(emp);break;}
			default:{System.out.println("Choose a better option, returning to menu");System.out.println("\n");EmpMenu(emp);break;}
			}
	...
	static void CreateAccount (String UserName1, String UserName2,String AccountType) {
		...
		Customer Tempcust = CDI.getCustomerByUsername(UserName1); adi.AutoApproveDeny(Tempcust);
